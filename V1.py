...
def create_brd_word_document(brd_content, output_filename):
    doc = Document()
    
    # Set up styles
    styles = doc.styles
    
    def create_style(name, font_size, bold=False, italic=False, alignment=WD_ALIGN_PARAGRAPH.LEFT):
    ...
    title_style = create_style('BRD Title', 18, bold=True, alignment=WD_ALIGN_PARAGRAPH.CENTER)
    heading1_style = create_style('BRD Heading 1', 16, bold=True)
    heading2_style = create_style('BRD Heading 2', 14, bold=True)
    normal_style = create_style('BRD Normal', 11)
    bullet_style = create_style('BRD Bullet', 11)
    bullet_style.paragraph_format.left_indent = Inches(0.25)
    bullet_style.paragraph_format.first_line_indent = Inches(-0.25)
    
    # Add a cover page
    doc.add_paragraph("Business Requirements Document", style=title_style)
    doc.add_page_break()
    
    # Add table of contents
    doc.add_paragraph("Table of Contents", style=heading1_style)
    doc.add_paragraph("Contents to be updated", style=normal_style)
    doc.add_page_break()
    
    # Process the BRD content
    ...
    for line in lines:
        ...
        if line.startswith('|') and line.endswith('|'):
            ...
            if line.startswith('**') and line.endswith('**'):
                ...
                p.runs[0].font.color.rgb = RGBColor(0, 0, 139)  # Dark Blue
            elif line.startswith('###'):
                # Subheading
                p = doc.add_paragraph(line.strip('# '), style=heading2_style)
                p.runs[0].font.color.rgb = RGBColor(0, 0, 139)  # Dark Blue
            elif line.startswith('*') or line.startswith('+'):
                # Bullet point
                p = doc.add_paragraph(line.strip('* +'), style=bullet_style)
                ...
                # Normal text with potential inline formatting
                p = doc.add_paragraph(style=normal_style)
...
# Groq client setup
groq_client = groq.Groq(api_key="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")
...
def main():
    ...
    # Results section
        ...
        def display_formatted_brd(brd_content):
        ...
        formatted_brd = display_formatted_brd(st.session_state.final_brd["brd_document"])
        ...
        st.download_button(
            ...
            data=st.session_state.final_brd["brd_document"],
        ...
        create_brd_word_document(
            st.session_state.final_brd["brd_document"],
            output_filename
        )
